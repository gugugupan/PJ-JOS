...
page_init(void)
{
	...
	// free pages!
	page_free_list = NULL;
	size_t ignore_area_left  = PGNUM( ROUNDDOWN(IOPHYSMEM,PGSIZE) ) ;
	size_t ignore_area_right = PGNUM( PADDR( boot_alloc( 0 ) ) ) ;
	size_t i;
	for (i = 0; i < npages; i++) {
		pages[i].pp_ref = 0;
		if ( i == 0 ) continue;
		if ( ignore_area_left <= i && i < ignore_area_right ) continue;
...
pgdir_walk(pde_t *pgdir, const void *va, int create)
{
	...
	if ( *pgdir_entry & PTE_P )
	{
		...
		// Not exists and not create
		return NULL ;
	} else {
		// Not exists and create
		struct PageInfo *new_pgtbl_page = page_alloc( 1 ) ;
		if ( new_pgtbl_page == NULL ) return NULL ;
		new_pgtbl_page -> pp_ref = 1 ;
		*pgdir_entry = page2pa( new_pgtbl_page ) | PTE_W | PTE_U | PTE_P ;
		pte_t *pt_va = (pte_t *) KADDR( page2pa( new_pgtbl_page ) ) ;
		return pt_va + PTX( va ) ;
	}
	return NULL;
}
